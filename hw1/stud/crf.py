...
class CRF(nn.Module):
    ...
    def __score_sentence(self, features, tags, masks):
        ...
        # emission score
        emit_scores = features.gather(dim=2,
                                      index=tags.unsqueeze(-1)).squeeze(-1)

        # transition score
        start_tag = torch.full((B, 1),
                               self.start_idx,
                               dtype=torch.long,
                               device=tags.device)
        ...
        last_tag = tags.gather(dim=1,
                               index=masks.sum(1).long().unsqueeze(1)).squeeze(
                                   1)  # [B]
    ...
    def __viterbi_decode(self, features, masks):
        ...
        max_score = torch.full((B, C), IMPOSSIBLE,
    ...
    def __forward_algorithm(self, features, masks):
        ...
        for t in range(L):
            ...
            score_t = scores.unsqueeze(
                1
            ) + trans + emit_score_t  # [B, 1, C] + [1, C, C] + [B, C, 1] => [B, C, C]
